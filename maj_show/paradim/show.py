...
from maj_show.paradim.config.show_config import showconfig
...
class SSVEP:
    def __init__(self, trigger_controller):
        ...
        # 刺激事件
        self.stim_event: list = showconfig.STIMULATION_EVENT

        # trial开始trigger
        self.trial_start_trig: list = showconfig.TRIAL_START_TRIGGER

        # 刺激结束输出trigger
        self.trial_end_trig: int = showconfig.TRIAL_END_TRIGGER

        # block启动输出trigger
        self.block_start_trig: int = showconfig.BLOCK_START_TRIGGER

        # block结束输出trigger
        self.block_end_trig: int = showconfig.BLOCK_END_TRIGGER

        # 数据开始记录trigger
        self.record_start_trig: int = showconfig.RECORD_START_TRIGGER

        # 数据停止记录trigger
        self.record_end_trig: int = showconfig.RECORD_END_TRIGGER
        ...
        # 所有刺激目标的位置
        self.stim_target_pos = showconfig.STIM_TARGET_POSITION
        ...
        def init_step_func(self):
            ...
            # 加载初始化帧
            self.background = os.path.join(self.frames_file_path, 'background.png')
